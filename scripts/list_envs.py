...
All the environments are registered in the `XARM_systematic` extension. They start
...
import XARM_systematic.tasks  # noqa: F401

# PLACEHOLDER: Extension template (do not remove this comment)
with contextlib.suppress(ImportError):
    import XARM_systematic.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~XARM_systematic.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from XARM_systematic.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `XARM_systematic` extension."""
    ...
    if args_cli.show_presets:
        from XARM_systematic.utils.preset_cli import enumerate_task_presets
